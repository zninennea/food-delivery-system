...
<body class="bg-stone-50 text-gray-800 antialiased">
            ...
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden md:flex items-center space-x-1">
                    ...
                    <a href="{{ route('customer.menu') }}"
                        class="text-gray-600 hover:text-orange-600 hover:bg-gray-50 px-3 py-2 rounded-lg text-sm font-medium transition-colors">
                        <i class="fas fa-utensils mr-1"></i> Menu
                    </a>
                    <a href="{{ route('customer.cart.index') }}"
                        class="text-gray-600 hover:text-orange-600 hover:bg-gray-50 px-3 py-2 rounded-lg text-sm font-medium transition-colors relative">
                        <i class="fas fa-shopping-cart mr-1"></i> Cart
                        @if($cartCount > 0)
                            <span
                                class="absolute top-0 right-0 -mt-1 -mr-1 px-1.5 py-0.5 bg-red-500 text-white text-xs font-bold rounded-full animate-pulse">{{ $cartCount }}</span>
                        @endif
                    </a>
    ...
    <div class="relative pt-20 pb-32 flex items-center justify-center overflow-hidden">
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 text-center mt-10 fade-in">
            <span class="text-orange-400 font-bold tracking-[0.2em] uppercase text-xs sm:text-sm mb-2 block">Premium
                Japanese Dining</span>
            ...
            <a href="{{ route('customer.menu') }}"
                class="group relative inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-full font-bold overflow-hidden shadow-lg hover:shadow-orange-500/30 transition-all hover:-translate-y-1">
                <span>Order Now</span>
    ...
    <div class="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 pb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 fade-in">
            <a href="{{ route('customer.menu') }}"
                class="bg-white rounded-2xl p-6 shadow-xl shadow-black/5 hover-card border border-stone-100 group">
                <div class="flex items-center justify-between mb-4">
                    <div class="p-3 bg-orange-50 rounded-xl group-hover:bg-orange-500 transition-colors duration-300">
                        <i
                            class="fas fa-utensils text-2xl text-orange-500 group-hover:text-white transition-colors"></i>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <div class="text-left sm:text-right bg-stone-50 p-4 rounded-xl">
                                <div class="flex items-center text-yellow-400 mb-1">
                                    ...
                                    @for($i = 1; $i <= 5; $i++)
                                        <i
                                            class="fas fa-star{{ $i <= floor($averageRating) ? '' : ($i == ceil($averageRating) ? '-half-alt' : '-o') }}"></i>
                                    @endfor
                                    <span
                                        class="text-gray-900 font-bold ml-2">{{ number_format($averageRating, 1) }}</span>
                                </div>
                                <p class="text-xs text-gray-500">{{ $totalReviews }} Reviews</p>
                                <p class="text-xs text-green-600 font-medium mt-1"><i class="fas fa-door-open mr-1"></i>
                                    Open until 10:00 PM</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('customer.menu') }}"
                            class="text-orange-600 hover:text-orange-700 text-sm font-medium">View All <i
                                class="fas fa-arrow-right ml-1"></i></a>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        @foreach($featuredItems as $item)
                            <div
                                class="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden hover-card group">
                                <div class="relative h-48 overflow-hidden">
                                    ...
                                    @endif
                                    <div
                                        class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full shadow-sm">
                                        <span
                                            class="text-orange-600 font-bold text-sm">₱{{ number_format($item->price, 2) }}</span>
                                    </div>
                                </div>
                                <div class="p-5">
                                    ...
                                    <a href="{{ route('customer.menu-item', $item) }}"
                                        class="block w-full py-2 bg-stone-100 text-stone-600 text-center rounded-lg hover:bg-orange-600 hover:text-white transition-colors text-sm font-medium">
                                        View Details
                                    </a>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            @foreach($recentOrders as $order)
                                            <div
                                                class="bg-white rounded-xl p-5 border-l-4 {{ $order->status == 'delivered' ? 'border-green-500' : ($order->status == 'cancelled' ? 'border-red-500' : ($order->status == 'pending' ? 'border-yellow-500' : 'border-blue-500')) }} shadow-sm">
                                                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                                    <div>
                                ...
                                ($order->status == 'cancelled' ? 'bg-red-100 text-red-700' :
                                    ($order->status == 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700')) }}">
                                                        ...
                                                        <p class="text-sm text-gray-500">{{ $order->created_at->format('M j, Y • g:i A') }}
                                                        </p>
                                                    </div>
                                                        ...
                                                        <div class="flex gap-3 mt-1 text-sm">
                                                            @if(!in_array($order->status, ['delivered', 'cancelled']))
                                                                <a href="{{ route('customer.track-order', $order) }}"
                                                                    class="text-orange-600 hover:underline">Track</a>
                                                            @endif
                                                            @if($order->status == 'delivered')
                                                                @php $hasReviewed = \App\Models\Review::where('order_id', $order->id)->exists(); @endphp
                                                                @if(!$hasReviewed)
                                                                    <a href="{{ route('customer.reviews.create', $order) }}"
                                                                        class="text-gray-500 hover:text-gray-900">Review</a>
                                                                @else
                                                                    <span class="text-green-600 text-xs">Reviewed</span>
                                                                @endif
                                                            @endif
                                                        </div>
                                                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 sticky top-24">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('customer.reviews.index') }}"
                            class="text-xs font-bold text-orange-600 uppercase hover:underline">View All</a>
                    </div>
                                ...
                                <div class="border-b border-gray-100 last:border-0 pb-6 last:pb-0">
                                    <div class="flex items-center gap-3 mb-3">
                                        @php
                                            ...
                                            if ($review->customer) {
                                                $customerNameInitial = strtoupper(substr($review->customer->name, 0, 1));

                                                // Check for profile picture
                                                if ($review->customer->profile_picture) {
                                                    $customerAvatar = asset('storage/' . $review->customer->profile_picture);
                                                }
                                                // Check for Google avatar
                                                elseif ($review->customer->oauth_provider === 'google' && $review->customer->avatar) {
                                        ...
                                        @else
                                            <div
                                                class="w-10 h-10 rounded-full bg-gradient-to-r from-orange-400 to-red-500 flex items-center justify-center text-white font-bold text-sm">
                                                {{ $customerNameInitial }}
                                            </div>
                                        @endif
                                    ...
                                    @if($review->comment)
                                        <p class="text-sm text-gray-600 italic bg-stone-50 p-3 rounded-lg mb-2">
                                            "{{ $review->comment }}"</p>
                                    @endif
                                    ...
                                    @if($review->order && $review->order->items->count() > 0)
                                        <div class="flex flex-wrap gap-1">
                                            @foreach($review->order->items->take(2) as $item)
                                                <span class="px-2 py-0.5 bg-stone-100 text-stone-500 text-[10px] rounded-full">
                                                    {{ $item->menuItem->name ?? 'Item' }}
                                                </span>
                                            @endforeach
                                            @if($review->order->items->count() > 2)
                                                <span
                                                    class="px-2 py-0.5 bg-stone-100 text-stone-500 text-[10px] rounded-full">+{{ $review->order->items->count() - 2 }}
                                                    more</span>
                                            @endif
                                        </div>
                                    @endif
                    ...
                    @else
                        <div class="text-center py-8 text-gray-400">
                            ...
                            <p class="text-xs text-gray-300 mt-1">Be the first to leave a review!</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <footer class="bg-stone-900 text-white py-12 border-t border-stone-800">
        <div class="max-w-7xl mx-auto px-4 text-center">
            ...
            <div class="flex justify-center gap-6 mb-8">
                <a href="#" class="text-stone-500 hover:text-white transition-colors"><i
            ...
            <p class="text-stone-600 text-xs">&copy; {{ date('Y') }} {{ $restaurant->name ?? 'NaNi Restaurant' }}. All
                rights reserved.</p>
        </div>
    </footer>
