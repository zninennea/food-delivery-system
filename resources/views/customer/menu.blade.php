...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="{{ route('customer.menu') }}" class="text-orange-600 px-3 py-2 rounded-md text-sm font-medium">
                    ...
                    <a href="{{ route('customer.profile.show') }}" class="text-gray-700 hover:text-orange-600 px-3 py-2 rounded-md text-sm font-medium">
    ...
    <div class="max-w-7xl mx-auto py-8 px-4">
        ...
        <div class="flex overflow-x-auto space-x-4 mb-8 pb-2">
            <button class="category-filter px-4 py-2 bg-orange-500 text-white rounded-full text-sm font-medium whitespace-nowrap" data-category="all">
                All Items
            </button>
            @foreach($menuItemsByCategory->keys() as $category)
            <button class="category-filter px-4 py-2 bg-gray-200 text-gray-700 rounded-full text-sm font-medium hover:bg-gray-300 whitespace-nowrap" data-category="{{ $category }}">
                {{ ucfirst($category) }}
            </button>
            @endforeach
        </div>
        ...
        <div class="mb-12 category-section" data-category="{{ $category }}">
            <h2 class="text-2xl font-bold text-gray-900 mb-6 border-b-2 border-orange-500 pb-2">{{ ucfirst($category) }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($items as $item)
                <div class="bg-white rounded-lg shadow hover:shadow-md transition duration-200 menu-item">
                    @if($item->image)
                        <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name }}" class="w-full h-48 object-cover rounded-t-lg">
                    @else
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex justify-between items-center mt-3">
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('customer.menu.item', $item) }}" class="bg-gray-500 text-white px-3 py-1 rounded-md text-sm hover:bg-gray-600 transition duration-200">
                                    Details
                                </a>
                                <button class="add-to-cart-btn bg-orange-500 text-white px-3 py-1 rounded-md text-sm hover:bg-orange-600 transition duration-200" 
    ...
    <div class="fixed bottom-4 right-4 bg-orange-500 text-white p-4 rounded-lg shadow-lg">
        <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('customer.cart.index') }}" class="bg-white text-orange-500 px-3 py-1 rounded text-sm font-medium hover:bg-gray-100">
                View Cart
            </a>
        </div>
    </div>
        ...
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <form id="addToCartForm">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center space-x-4">
                        <button type="button" id="decreaseQty" class="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">-</button>
                        <input type="number" id="quantity" name="quantity" value="1" min="1" max="10" 
                               class="w-16 text-center border border-gray-300 rounded-md py-1">
                        <button type="button" id="increaseQty" class="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">+</button>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" id="closeModal" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                        Cancel
                    </button>
